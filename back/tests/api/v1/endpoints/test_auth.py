...
class TestPlatonInit:
    ...
    async def test_returns_redirect_url_and_state(self):
        ...
        with patch("src.api.v1.endpoints.auth.auth_service") as mock_auth:
            with patch("src.api.v1.endpoints.auth.settings") as mock_settings:
                mock_auth.generate_oauth_state = AsyncMock(return_value="state-abc")
                mock_settings.PLATON_LOGIN_BASE_URL = "https://platon.example/login"
                mock_settings.PLATON_CALLBACK_URL = "https://app.example/callback"
                mock_settings.PLATON_CALLBACK_TITLE = "Agora"
    ...
    async def test_invalid_token_returns_401(self):
        ...
        with patch("src.api.v1.endpoints.auth.auth_service") as mock_auth:
            with patch("src.api.v1.endpoints.auth.settings") as mock_settings:
                ...
                mock_settings.PLATON_PUBLIC_KEY = "key"
