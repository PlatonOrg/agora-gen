...
| 3 | **Monitoring en production** | Moyenne | Pas de monitoring applicatif centralisé (health checks, alertes, métriques Prometheus). |
| 4 | **Le frontend ne gère pas l'expiration de session** | Basse | Pas de détection proactive de l'expiration de session côté client. L'utilisateur est redirigé vers la page de connexion seulement lors d'une prochaine requête échouée. |
...
|---|-------------|--------|--------|
| 1 | **Tests E2E avec Playwright** | Qualité, non-régression | 1 semaine |
| 2 | **Monitoring Prometheus + Grafana** | Observabilité production | 3-5 jours |
| 3 | **Retry SSE côté client** | Résilience en cas de déconnexion réseau | 1-2 jours |
| 4 | **Suppression des ressources obsolètes lors de la sync** | Cohérence des données RAG | 2-3 jours |
| 5 | **RAG et sélection de composants lors des modifications** | Qualité des générations de modification | 3-5 jours |

### 2.2 Moyen terme

| # | Amélioration | Impact | Effort |
|---|-------------|--------|--------|
| 1 | **Système de feedback utilisateur** | Amélioration qualité des prompts | 1 semaine |
